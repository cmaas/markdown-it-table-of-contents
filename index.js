...
function findHeadlineElements(levels, tokens, options) {
  ...
  tokens.forEach(token => {
    if (token.type === 'heading_open') {
      var level = parseInt(token.tag.toLowerCase().replace('h', ''), 10);
      // TODO: add support for markdown-it-attrs like so: `# Article {#myslug}` -> `<h1 id="myslug">Article</h1>`
      // markdown-it-attrs could be used as default for the TocItem.anchor prop
      /* var attrId = token.attrs?.length > 0 ? token.attrs.find( attr => attr[0] === 'id' ) : null; */
      ...
      var textContent = token.children
        .filter((childToken) => childToken.type === 'text' || childToken.type === 'code_inline')
        .reduce((acc, t) => acc + t.content, '');
      var slugifiedContent = options.slugify(textContent, token.content);
      currentHeading.text = textContent;
      currentHeading.anchor = slugifiedContent;
    }
